...
void DoLighting(inout vec4 color, inout vec3 shadowMult, vec3 playerPos, vec3 viewPos, float lViewPos, vec3 geoNormal, vec3 normalM, float dither,
    ...
    #if defined PHOTONICS_LIGHTING && !defined GBUFFERS_WATER && !defined GBUFFERS_HAND && defined PHOTONICS_COMBINED_GI && PHOTONICS_LIGHTING_MODE == 2 && PHOTONICS_VERSION >= 303
        lightmap.y *= mix(1.0, 0.0, phRtCoverage * sunVisibility);
    #endif
    ...
    // Shadows
        ...
        shadowMult = mix(vec3(1.0), shadowMult * END_FLASH_SHADOW * 0.1, endFlashIntensityM);
    #endif
    ...
    // Lighting Tweaks
        ...
        ambientMult *= 1.0 - mix(0.0, 0.367 * photonicsAmbientLightMult, PHOTONICS_INDIRECT_INTENSITY * 0.05) * phRtCoverage;
    #endif
